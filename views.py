...
def handle_booking_payment():
    ...
    try:
        ...
        # Calculate reward coins (10% of booking amount)
        reward_coins = int(amount * 0.1)
        ...
        new_balance = current_balance + reward_coins
        ...
        transaction = Transaction(
            ...
            transaction_id=f'REWARD_{booking_id}'
        )
...
def process_booking_payment():
    ...
    try:
        ...
        db.session.flush()  # Get the booking ID before commit
        
        # Calculate reward coins (10% of booking amount)
        reward_coins = int(float(booking_data.get('amount', 0)) * 0.1)
        ...
        transaction = Transaction(
            ...
            amount=reward_coins,
            transaction_type='reward',
            transaction_id=f'REWARD_{new_booking.id}'
        )
        db.session.add(transaction)
...
def admin_users():
    ...
    for user in users:
        user_data.append({
            ...
            'itihaas_coins': user.itihaas_coins.coins if user.itihaas_coins else 0
        })
    
    return jsonify({
        'users': user_data,
        'total_users': len(user_data),
        'verified_users': len([u for u in user_data if u['is_verified']]),
        'pending_verifications': len([u for u in user_data if not u['is_verified']]),
        'guides': len([u for u in user_data if u['is_guide']])
    })
...
def admin_bookings():
    ...
    for booking in bookings:
        booking_data.append({
            'id': booking.id,
            'user_name': booking.user.first_name if booking.user else 'N/A',
            'monument_name': booking.monument_name,
            'booking_date': booking.booking_date.strftime('%Y-%m-%d'),
            'visitors': booking.visitors,
            'total_amount': booking.total_amount,
            'status': booking.status,
            'guide_name': booking.guide.name if booking.guide else 'No Guide'
        })
    
    return jsonify({
        'bookings': booking_data,
        'total_bookings': len(booking_data),
        'pending_approvals': len([b for b in booking_data if b['status'] == 'Pending']),
        'today_bookings': len([b for b in booking_data if b['booking_date'] == datetime.now().strftime('%Y-%m-%d')])
    })
#10000000